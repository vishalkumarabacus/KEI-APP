...
export class AddLeavePage {
  ...
  add_leave()
  {
    ...
    this.db.addData({'data':this.data},"leave/add_leave").then(response=>
      {
        ...
        if(response=="exist more")
        {
          let alert=this.alertCtrl.create({
            // title:'Exists !!',
            subTitle: 'You Already Applied Short Leave 2 Times in This Month',
    ...
    {
      let toast1 = this.toastCtrl.create({
        message: 'Leave Already Exists of Same Date',
        duration: 3000,
        position: 'bottom'
      });
    ...
    {
      let toast1 = this.toastCtrl.create({
        message: 'You Already Applied Shortleave',
        duration: 3000,
        position: 'bottom'
      });
    ...
    getleaveList()
    {
      this.db.addData({'type':this.data.type},'Leave/leaves_type').then((result)=>
      {
        console.log(result);
